...
int main(){
    ...
    {
        if(!(i % 2 == 0)){ // Run if i is odd
            odd_term += 2; // Updating odd term
        }
        else{ // Run if i is even
    ...
    if(n % 2 == 0) printf("The nth term is: %d", even_term); // if n is even
    else printf("THe nth term is: %d", odd_term); // if n is odd
